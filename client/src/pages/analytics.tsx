...
export default function Analytics() {
  ...
  const availableYears = (availablePeriods as PeriodData[]).length > 0 ? 
    [...new Set((availablePeriods as PeriodData[]).map((p: PeriodData) => p.year))].sort().reverse() :
  ...
  const availableMonths = (availablePeriods as PeriodData[]).length > 0 && selectedYear ? 
    (availablePeriods as PeriodData[])
      .filter((p: PeriodData) => p.year === selectedYear)
      .map((p: PeriodData) => ({ value: p.month, label: p.monthName })) :
          ...
          <div className="flex items-center space-x-2">
            ...
            <Select value={selectedYear} onValueChange={(year) => {
              ...
              if ((availablePeriods as PeriodData[]).length > 0) {
                const firstMonthInYear = (availablePeriods as PeriodData[]).find((p: PeriodData) => p.year === year);
                if (firstMonthInYear) {
