...
export default function Candidates() {
  ...
  const getStageProgress = (candidate: Candidate) => {
    if (!candidate.interviewStageChain || candidate.interviewStageChain.length === 0) {
      return { current: 0, total: 0, stages: [] };
    }
    const passed = candidate.interviewStageChain.filter((s: InterviewStage) => s.status === 'passed').length;
    const stages = candidate.interviewStageChain.map((s: InterviewStage) => ({
      name: s.stageName,
      status: s.status,
    }));
    return { current: passed, total: candidate.interviewStageChain.length, stages };
  };
